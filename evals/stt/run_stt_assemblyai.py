...
STT_SCRIPT = Path("../../stt_assemblyai.py")
...
def main():
    parser = argparse.ArgumentParser(
        description="Smoke-test ../../stt_assemblyai.py against local audio samples."
    )
    parser.add_argument(
        "--min-ratio",
        type=float,
    ...
    parser.add_argument(
        ...
        choices=["eu", "us"],
        default=None,
        help="AssemblyAI region to pass through (eu/us).",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--keep-json",
        action="store_true",
        help="Keep *.assemblyai.json files next to audio.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not os.environ.get("ASSEMBLYAI_API_KEY"):
        print("SKIP: ASSEMBLYAI_API_KEY is not set.")
        return 2
    ...
    results_dir = SCRIPT_DIR / "results" / "assemblyai"
    ...
    for stem, sample_path, voice, audio_path in cases:
        ...
        json_path = audio_path.with_name(audio_path.name + ".assemblyai.json")
        ...
        if proc.returncode != 0:
            failed += 1
            print(f"FAIL: {stem} ({voice}) - stt_assemblyai.py returned {proc.returncode}")
            if args.verbose:
                if proc.stdout:
