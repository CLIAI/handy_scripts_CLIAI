...
STT_SCRIPT = Path("../../stt_speechmatics.py")
...
def main():
    parser = argparse.ArgumentParser(
        description="Smoke-test ../../stt_speechmatics.py against local audio samples."
    )
    parser.add_argument(
        "--min-ratio",
        type=float,
    ...
    parser.add_argument(
        "--region",
        choices=["eu", "eu1", "us", "us1", "au", "au1"],
        default=None,
        help="Speechmatics region to pass through.",
    )
    parser.add_argument(
        "--operating-point",
        choices=["standard", "enhanced"],
        default="enhanced",
        help="Model accuracy: standard (faster) or enhanced (more accurate). Default: enhanced.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--keep-json",
        action="store_true",
        help="Keep *.speechmatics.json files next to audio.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not os.environ.get("SPEECHMATICS_API_KEY"):
        print("SKIP: SPEECHMATICS_API_KEY is not set.")
        return 2
    ...
    results_dir = SCRIPT_DIR / "results" / "speechmatics"
    ...
    for stem, sample_path, voice, audio_path in cases:
        ...
        json_path = audio_path.with_name(audio_path.name + ".speechmatics.json")
        ...
        if proc.returncode != 0:
            failed += 1
            print(f"FAIL: {stem} ({voice}) - stt_speechmatics.py returned {proc.returncode}")
            if args.verbose:
                if proc.stdout:
